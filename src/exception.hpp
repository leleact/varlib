...
public:
  exception()
      : m_file_(""), m_line_(0), m_func_(""), m_mess_(""), m_errno_(errno) {
    build_what_message();
  }
  exception(const std::string &_Msg)
      : m_file_(""), m_line_(0), m_func_(""), m_mess_(_Msg), m_errno_(errno) {
    build_what_message();
  }
  exception(const std::string &_file, const std::size_t &_line)
      : m_file_(_file), m_line_(_line), m_func_(""), m_mess_(""),
        m_errno_(errno) {
    build_what_message();
  }
  exception(const std::string &_file, const std::size_t &_line,
            const std::string &_func)
      : m_file_(_file), m_line_(_line), m_func_(_func), m_mess_(""),
        m_errno_(errno) {
    build_what_message();
  }
  exception(const std::string &_file, const std::size_t &_line,
            const std::string &_func, const std::string &_message)
      : m_file_(_file), m_line_(_line), m_func_(_func), m_mess_(_message),
        m_errno_(errno) {
    build_what_message();
  }
...
#define VARLIB_THROW_DEFAULT                                                   \
  { throw varlib::exception(__FILE__, __LINE__, __PRETTY_FUNCTION__); }
