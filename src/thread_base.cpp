...
int thread_base::join() { return pthread_join(m_pid, NULL); }

int thread_base::detach() { return pthread_detach(m_pid); }

int thread_base::stop() { return pthread_join(m_pid, NULL); }

thread_base::thread_id thread_base::get_thread_id() {
  return static_cast<thread_id>(m_pid);
}

int thread_base::get_thread_rc() { return m_thread_rc; }
